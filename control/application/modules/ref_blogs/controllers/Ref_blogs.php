...
class Ref_blogs extends MX_Controller  {
	
	var $path = "ref_blogs";
	var $alias = "Blog/News Categories";	
	var $uri_page = 6;
	var $per_page = 25;
	...
	{	
		...
		$jum_record = $this->ref_blogs->getTotal($sch_pack);
		...
		#record
		$query = $this->ref_blogs->getList($per_page,$lmt,$sch_pack);
		$list = array();
	...
	function submit()
	{
		...
		if ($this->form_validation->run($this) == FALSE)
		{
			...
			if($id > 0)
			{
				$this->ref_blogs->setUpdate($data_pack,$where_pack);
				$this->session->set_flashdata("success","Data saved successful");
				redirect($this->path."/edit/".$id);
			}else{
				$id_term = $this->ref_blogs->setInsert($data_pack);
	...
	{
		$del_status = $this->ref_blogs->setDelete($id);
		$response['id'] = $id;
