...
class Model_partners extends CI_Model {
	...
	function setFileUpload($file_image,$file_image_tmp,$file_image_old)
	{
		...
		$h = 194;					
 		...
 		$this->image_moo->load($file_image_tmp)->resize_crop($w,$h)->save("../uploads/partners/".$file_image);
		$this->image_moo->load($file_image_tmp)->resize_crop(167,47)->save("../uploads/partners/thumbs/".$file_image);
	...
	{
		if(!empty($file_image)){
			if(file_exists("../uploads/partners/".$file_image)){ unlink("../uploads/partners/".$file_image); }
			if(file_exists("../uploads/partners/thumbs/".$file_image)){ unlink("../uploads/partners/thumbs/".$file_image); }
		}
	}
