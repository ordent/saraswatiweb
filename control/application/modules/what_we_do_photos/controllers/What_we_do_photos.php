...
class What_we_do_photos extends MX_Controller  {
	
	var $path = "what_we_do_photos";
	var $alias = "What We Do - Photos";	
	var $uri_page = 7;
	var $per_page = 25;
	...
	{	
		...
		$jum_record = $this->what_we_do_photos->getTotal($sch_pack);
		...
		#record
		$query = $this->what_we_do_photos->getList($per_page,$lmt,$sch_pack);
		$list = array();
	...
	function edit_content($id)
	{
		...
		if(is_numeric($id)){
			...
			#record
			$q = $this->what_we_do_photos->getDetail($id);
	...
	function submit()
	{
		...
		if(!empty($file_image_title)){
			if(!empty($file_image_old)){
				$this->what_we_do_photos->deleteFileUpload($file_image_old);
			}
			$file_image = $this->what_we_do_photos->setFileUpload($file_image_title,$file_image_tmp,$file_image_old);
		}else{
		...
		if ($this->form_validation->run($this) == FALSE)
		{
			...
			if($id > 0)
			{
				$this->what_we_do_photos->setUpdate($data_pack,$where_pack);
				$this->session->set_flashdata("success","Data saved successful");
				$link_id = $id;
			}else{
				$id_term = $this->what_we_do_photos->setInsert($data_pack);
	...
	{
		$product = $this->what_we_do_photos->getDetail($id);
		$row = $product->row();
		$file_image = $row->file_image;
		$file_thumb = $row->file_thumb;
		$del_images = $this->what_we_do_photos->setPictureDelete($id,$file_image);
		$del_status = $this->what_we_do_photos->setDelete($id);
		$response['id'] = $id;
	...
	function unlink($id,$file_image)
	{
		...
		$this->what_we_do_photos->setPictureDelete($id, $file_image);
		redirect($this->path."/edit/".$id);
	}
