...
class Blogs extends MX_Controller  {
	...
	{	
		...
		$q_ref3 = Modules::run('widget/getQueryStaticDropdown','ref_blogs','name', $wheres3);
		$ref3_arr = array();
		...
		$sch4_parm = $sch4_parm != 'null' ? $sch4_parm : 'null';
		$sch4_parm =  $sch4_parm == ""  ? 'null' : $sch4_parm;
		$sch4_arr = array(
							  0=>"No",
							  1=>"Yes"
							  );
		$sch4_select_arr = array($sch4_parm);
		$ref4 = Modules::run('widget/getStaticDropdown',$sch4_arr,$sch4_select_arr,4);
		...
		$jum_record = $this->blogs->getTotal($sch_pack);
		...
		#record
		$query = $this->blogs->getList($per_page,$lmt,$sch_pack);
		$list = array();
		...
		if($query->num_rows() > 0){
			foreach($query->result() as $r)
			{
				...
				$is_highlight = $r->is_highlight == 1 ? "icon-ok-sign" : "icon-minus-sign";
				...
				$list[] = array(
								...
								"id"=>$r->id,
								"name"=>$title,
								"ref_blogs_name"=>$r->ref_blogs_name,
	...
	function edit_content($id)
	{
		...
		if(is_numeric($id)){
			...
			$ref4_arr = array(0=>"Normal",1=>"Highlight");

			#record
			$q = $this->blogs->getDetail($id);
			...
			if($q->num_rows() > 0){
					...
					{
						$file_thumb = "
						<a href='".asset_url()."blogs/".$r->file_thumb."' rel='facebox'><img src='".asset_url()."blogs/thumbs/".$r->file_thumb."' class='thumbnail'></a>
						<br/>
						<a href='".base_url().$this->path."/unlink_thumb/".$r->id."/".rawurlencode($r->file_thumb)."' class='btn btn-info'>delete</a><br/>
						<input type='hidden' name='file_thumb_old' value='".$r->file_thumb."' id='file_thumb_old'>";
					}
					#end image
					...
					{
						$file_image = "
						<a href='".asset_url()."blogs/".$r->file_image."' rel='facebox'><img src='".asset_url()."blogs/thumbs/".$r->file_image."' class='thumbnail'></a>
						<br/>
					...
					{
						$file_doc = "
						<a href='".asset_url()."blogs/docs/".$r->file_doc."'>".$r->file_doc."</a>
						<br/>
						<a href='".base_url().$this->path."/unlink_document/".$r->id."/".rawurlencode($r->file_doc)."' class='btn btn-info'>delete</a><br/>
						<input type='hidden' name='file_doc_old' value='".$r->file_doc."' id='files_old'>";
					}
					#end image
					...
					$author = $this->session->flashdata("author") ? $this->session->flashdata("author") : $r->author;
					...
					foreach ($q_result3 as $r_ref3) {
						 $ref3_select_arr[$r3] = $this->session->flashdata("ref3") ? $r_ref3 : $r_ref3->ref_blogs_id;
						 $r3++;
					}
					...
					#ref dropdown no multi value
					$ref4_select_arr[0] = $this->session->flashdata("is_highlight") ? $this->session->flashdata("is_highlight") : $r->is_highlight;
					...
					$author = $this->session->flashdata("author") ? $this->session->flashdata("author") : "";
					...
					#ref dropdown no multi value
					$ref4_select_arr[0] = $this->session->flashdata("is_highlight") ? $this->session->flashdata("is_highlight") : null;
					$ref4 = Modules::run('widget/getStaticDropdown',$ref4_arr,$ref4_select_arr,4);
					#end ref dropdown no multi value
	...
	function submit()
	{
		...
		if(!empty($file_image_title)){
			if(!empty($file_image_old)){
				$this->blogs->deleteFileUpload($file_image_old);
			}
			$file_image = $this->blogs->setFileUpload($file_image_title,$file_image_tmp,$file_image_old);
		}else{
			$file_image = $file_image_old;
		}

		#image 
		$file_thumb_old = strip_tags($this->input->post("file_thumb_old"));
		$file_thumb_title = $_FILES["file_thumb"]["name"];
		$file_thumb_tmp  = $_FILES["file_thumb"]["tmp_name"];
		
		if(!empty($file_thumb_title)){
			if(!empty($file_thumb_old)){
				$this->blogs->deleteFileUploadThumb($file_thumb_old);
			}
			$file_thumb = $this->blogs->setFileUploadThumb($file_thumb_title,$file_thumb_tmp,$file_thumb_old);
		}else{
			$file_thumb = $file_thumb_old;
		}

		#document
		$file_doc_old = strip_tags($this->input->post("file_doc_old"));
		$file_doc_title = $_FILES["file_doc"]["name"];
		$file_doc_tmp = $_FILES["file_doc"]["tmp_name"];

		if(!empty($file_doc_title)){
			if(!empty($file_doc_old)){
				$this->blogs->deleteDocumentUpload($file_doc_old);
			}
			$file_doc = $this->blogs->setDocumentUpload($file_doc_title,$file_doc_tmp,$file_doc_old);
		}else{
			$file_doc = $file_doc_old;
		}
		...
		$data_pack = array(
						...
						'preview'=>strip_tags($this->input->post("preview")),
						'description'=>strip_tags($this->input->post("description"),$tags),
						"ref_blogs_id"=>strip_tags($this->input->post("ref3")),
						'file_thumb'=>$file_thumb,
						'file_image'=>$file_image,
						'file_doc'=>$file_doc,
						'is_highlight'=>$this->input->post("ref4"),
		...
		if ($this->form_validation->run($this) == FALSE)
		{
			...
			if($id > 0)
			{
				$this->blogs->setUpdate($data_pack,$where_pack);
				$this->session->set_flashdata("success","Data saved successful");
				$link_id = $id;
			}else{
				$id_term = $this->blogs->setInsert($data_pack);
	...
	{
		$product = $this->blogs->getDetail($id);
		$row = $product->row();
		$file_image = $row->file_image;
		$file_thumb = $row->file_thumb;
		$del_images = $this->blogs->setPictureDelete($id,$file_image);
		$del_thumb = $this->blogs->setPictureDeleteThumb($id,$file_thumb);
		$del_status = $this->blogs->setDelete($id);
		$response['id'] = $id;
	...
	function unlink($id,$file_image)
	{
		...
		$this->blogs->setPictureDelete($id, $file_image);
		redirect($this->path."/edit/".$id);
	}

	function unlink_thumb($id,$file_image)
	{
		$file_image = rawurldecode($file_image);
		$this->blogs->setPictureDeleteThumb($id, $file_image);
		redirect($this->path."/edit/".$id);
	}
	...
	function unlink_document($id,$file)
	{
		...
		$this->blogs->setDocumentDelete($id, $file);
		redirect($this->path."/edit/".$id);
	}
