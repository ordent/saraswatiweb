...
class Programs extends MX_Controller  {
	...
	{	
		...
		$jum_record = $this->programs->getTotal($sch_pack);
		...
		#record
		$query = $this->programs->getList($per_page,$lmt,$sch_pack);
		$list = array();
	...
	function submit()
	{
		...
		if(!empty($file_image_title)){
			if(!empty($file_image_old)){
				$this->programs->deleteFileUpload($file_image_old);
			}
			$file_image = $this->programs->setFileUpload($file_image_title,$file_image_tmp,$file_image_old);
		}else{
		...
		if(!empty($file_thumb_title)){
			if(!empty($file_thumb_old)){
				$this->programs->deleteFileUploadThumb($file_thumb_old);
			}
			$file_thumb = $this->programs->setFileUploadThumb($file_thumb_title,$file_thumb_tmp,$file_thumb_old);
		}else{
		...
		if ($this->form_validation->run($this) == FALSE)
		{
			...
			if($id > 0)
			{
				$this->programs->setUpdate($data_pack,$where_pack);
				$this->session->set_flashdata("success","Data saved successful");
				$link_id = $id;
			}else{
				$id_term = $this->programs->setInsert($data_pack);
	...
	{
		$product = $this->programs->getDetail($id);
		$row = $product->row();
		$file_image = $row->file_image;
		$file_thumb = $row->file_logo;
		$del_images = $this->programs->setPictureDelete($id,$file_image);
		$del_thumb = $this->programs->setPictureDeleteThumb($id,$file_thumb);
		$del_status = $this->programs->setDelete($id);
		$response['id'] = $id;
	...
	function unlink($id,$file_image)
	{
		...
		$this->programs->setPictureDelete($id, $file_image);
		redirect($this->path."/edit/".$id);
	}
	...
	{
		$file_image = rawurldecode($file_image);
		$this->programs->setPictureDeleteThumb($id, $file_image);
		redirect($this->path."/edit/".$id);
	}
	...
	function unlink_document($id,$file)
	{
		...
		$this->programs->setDocumentDelete($id, $file);
		redirect($this->path."/edit/".$id);
	}
