...
class Model_forms extends CI_Model {
	...
	function getTotal($sch_pack)
	{
		if($sch_pack){
			...
			{
				$this->db->like($this->table.'.ref_forms_id',$sch_pack['sch3_parm']);
			}
		}
	...
	function getList($per_page,$lmt,$sch_pack)
	{
		...
		$this->db->select($this->table.".*,
						 (SELECT name FROM tbl_".$this->tbl_ref_forms." WHERE tbl_".$this->tbl_ref_forms.".id = tbl_".$this->table.".ref_forms_id) AS ref_forms_name
						 ");
	...
	function setFileUpload($file_image,$file_image_tmp,$file_image_old)
	{
		...
		$h = 400;					
 		...
 		$this->image_moo->load($file_image_tmp)->resize_crop($w,$h)->save("../uploads/forms/".$file_image);
 		$this->image_moo->load($file_image_tmp)->resize_crop(400,200)->save("../uploads/forms/medium/".$file_image);
		$this->image_moo->load($file_image_tmp)->resize_crop(200,100)->save("../uploads/forms/thumbs/".$file_image);
	...
	{
		if(!empty($file_image)){
			if(file_exists("../uploads/forms/".$file_image)){ unlink("../uploads/forms/".$file_image); }
			if(file_exists("../uploads/forms/medium/".$file_image)){ unlink("../uploads/forms/medium/".$file_image); }
			if(file_exists("../uploads/forms/thumbs/".$file_image)){ unlink("../uploads/forms/thumbs/".$file_image); }
		}
	}
	...
	function setDocumentUpload($file,$file_tmp,$file_old)
	{
		...
		if (!move_uploaded_file($file_tmp, "../uploads/forms/files/".$file)) {
			return false;
		}
	...
	{
		if(!empty($file)){
			if(file_exists("../uploads/forms/files/".$file)){ unlink("../uploads/forms/files/".$file); }
		}
	}
