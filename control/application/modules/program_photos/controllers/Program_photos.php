...
class Program_photos extends MX_Controller  {
	
	var $path = "program_photos";
	var $alias = "Program IG Photos";	
	var $uri_page = 7;
	var $per_page = 25;
	...
	{	
		...
		$q_ref3 = Modules::run('widget/getQueryStaticDropdown','programs','name', $wheres3);
		$ref3_arr = array();
		...
		$jum_record = $this->program_photos->getTotal($sch_pack);
		...
		#record
		$query = $this->program_photos->getList($per_page,$lmt,$sch_pack);
		$list = array();
	...
	function edit_content($id)
	{
		...
		if(is_numeric($id)){
			...
			#record
			$q = $this->program_photos->getDetail($id);
			...
			if($q->num_rows() > 0){
					...
					foreach ($q_result3 as $r_ref3) {
						 $ref3_select_arr[$r3] = $this->session->flashdata("ref3") ? $r_ref3 : $r_ref3->programs_id;
						 $r3++;
					}
	...
	function submit()
	{
		...
		if(!empty($file_image_title)){
			if(!empty($file_image_old)){
				$this->program_photos->deleteFileUpload($file_image_old);
			}
			$file_image = $this->program_photos->setFileUpload($file_image_title,$file_image_tmp,$file_image_old);
		}else{
		...
		$data_pack = array(
						'programs_id'=>strip_tags($this->input->post("ref3")),
		...
		if ($this->form_validation->run($this) == FALSE)
		{
			...
			if($id > 0)
			{
				$this->program_photos->setUpdate($data_pack,$where_pack);
				$this->session->set_flashdata("success","Data saved successful");
				$link_id = $id;
			}else{
				$id_term = $this->program_photos->setInsert($data_pack);
	...
	{
		$product = $this->program_photos->getDetail($id);
		$row = $product->row();
		$file_image = $row->file_image;
		$del_images = $this->program_photos->setPictureDelete($id,$file_image);
		$del_status = $this->program_photos->setDelete($id);
		$response['id'] = $id;
	...
	function unlink($id,$file_image)
	{
		...
		$this->program_photos->setPictureDelete($id, $file_image);
		redirect($this->path."/edit/".$id);
	}
