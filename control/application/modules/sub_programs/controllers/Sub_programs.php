...
class Sub_programs extends MX_Controller  {
	
	var $path = "sub_programs";
	var $alias = "Sub Programs";	
	var $uri_page = 6;
	var $per_page = 25;
	...
	{	
		...
		$jum_record = $this->sub_programs->getTotal($sch_pack);
		...
		#record
		$query = $this->sub_programs->getList($per_page,$lmt,$sch_pack);
		$list = array();
	...
	function edit_content($id)
	{
		...
		if(is_numeric($id)){
			...
			$ref3_arr = array("light"=>"Light","dark"=>"Dark");

			#record
			$q = $this->sub_programs->getDetail($id);
			...
			if($q->num_rows() > 0){
					...
					{
						$file_thumb = "
						<a href='".asset_url()."programs/".$r->file_logo."' rel='facebox'><img src='".asset_url()."programs/thumbs/".$r->file_logo."' class='thumbnail'></a>
						<br/>
						<a href='".base_url().$this->path."/unlink_thumb/".$r->id."/".rawurlencode($r->file_logo)."' class='btn btn-info'>delete</a><br/>
						<input type='hidden' name='file_thumb_old' value='".$r->file_logo."' id='file_thumb_old'>";
					}
					#end image
					...
					{
						$file_image = "
						<a href='".asset_url()."programs/".$r->file_image."' rel='facebox'><img src='".asset_url()."programs/thumbs/".$r->file_image."' class='thumbnail'></a>
						<br/>
					...
					$slug = $this->session->flashdata("slug") ? $this->session->flashdata("slug") : $r->slug;
					...
					#ref dropdown no multi value
					$ref3_select_arr[0] = $this->session->flashdata("theme") ? $this->session->flashdata("theme") : $r->theme;
					...
					$slug = $this->session->flashdata("slug") ? $this->session->flashdata("slug") : "";
					...
					#ref dropdown no multi value
					$ref3_select_arr[0] = $this->session->flashdata("theme") ? $this->session->flashdata("theme") : null;
					$ref3 = Modules::run('widget/getStaticDropdown',$ref3_arr,$ref3_select_arr,3);
					#end ref dropdown no multi value
	...
	function submit()
	{
		...
		if(!empty($file_image_title)){
			if(!empty($file_image_old)){
				$this->sub_programs->deleteFileUpload($file_image_old);
			}
			$file_image = $this->sub_programs->setFileUpload($file_image_title,$file_image_tmp,$file_image_old);
		}else{
		...
		if(!empty($file_thumb_title)){
			if(!empty($file_thumb_old)){
				$this->sub_programs->deleteFileUploadThumb($file_thumb_old);
			}
			$file_thumb = $this->sub_programs->setFileUploadThumb($file_thumb_title,$file_thumb_tmp,$file_thumb_old);
		}else{
		...
		$data_pack = array(
						...
						'slug'=>strtolower(str_replace(" ","",strip_tags($this->input->post("slug")))),
						'description'=>strip_tags($this->input->post("description"),$tags),
						'file_logo'=>$file_thumb,
						'file_image'=>$file_image,
						'is_enabled'=>$this->input->post("ref2"),
						'theme'=>$this->input->post("ref3"),
		...
		if ($this->form_validation->run($this) == FALSE)
		{
			...
			if($id > 0)
			{
				$this->sub_programs->setUpdate($data_pack,$where_pack);
				$this->session->set_flashdata("success","Data saved successful");
				$link_id = $id;
			}else{
				$id_term = $this->sub_programs->setInsert($data_pack);
	...
	{
		$product = $this->sub_programs->getDetail($id);
		$row = $product->row();
		$file_image = $row->file_image;
		$file_thumb = $row->file_logo;
		$del_images = $this->sub_programs->setPictureDelete($id,$file_image);
		$del_thumb = $this->sub_programs->setPictureDeleteThumb($id,$file_thumb);
		$del_status = $this->sub_programs->setDelete($id);
		$response['id'] = $id;
	...
	function unlink($id,$file_image)
	{
		...
		$this->sub_programs->setPictureDelete($id, $file_image);
		redirect($this->path."/edit/".$id);
	}
	...
	{
		$file_image = rawurldecode($file_image);
		$this->sub_programs->setPictureDeleteThumb($id, $file_image);
		redirect($this->path."/edit/".$id);
	}
	...
	function unlink_document($id,$file)
	{
		...
		$this->sub_programs->setDocumentDelete($id, $file);
		redirect($this->path."/edit/".$id);
	}
