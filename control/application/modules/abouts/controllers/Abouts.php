...
class Abouts extends MX_Controller  {
	...
	{	
		...
		$jum_record = $this->abouts->getTotal($sch_pack);
		...
		#record
		$query = $this->abouts->getList($per_page,$lmt,$sch_pack);
		$list = array();
	...
	function edit_content($id)
	{
		...
		if(is_numeric($id)){
			...
			if($q->num_rows() > 0){
					...
					{
						$file_image = "
						<a href='".asset_url()."abouts/".$r->file_image."' rel='facebox'><img src='".asset_url()."abouts/thumbs/".$r->file_image."' class='thumbnail'></a>
						<br/>
	...
	function submit()
	{
		...
		if(!empty($file_image_title)){
			if(!empty($file_image_old)){
				$this->abouts->deleteFileUpload($file_image_old);
			}
			$file_image = $this->abouts->setFileUpload($file_image_title,$file_image_tmp,$file_image_old);
		}else{
		...
		if ($this->form_validation->run($this) == FALSE)
		{
			...
			if($id > 0)
			{
				$this->abouts->setUpdate($data_pack,$where_pack);
				$this->session->set_flashdata("success","Data saved successful");
				$link_id = $id;
			}else{
				$id_term = $this->abouts->setInsert($data_pack);
	...
	{
		$product = $this->abouts->getDetail($id);
		$row = $product->row();
		$file_image = $row->file_image;
		$del_images = $this->abouts->setPictureDelete($id,$file_image);
		$del_status = $this->abouts->setDelete($id);
		$response['id'] = $id;
	...
	function unlink($id,$file_image)
	{
		...
		$this->abouts->setPictureDelete($id, $file_image);
		redirect($this->path."/edit/".$id);
	}
