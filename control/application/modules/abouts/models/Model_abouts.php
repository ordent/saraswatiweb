<?php
class Model_abouts extends CI_Model {
	...
	function setFileUpload($file_image,$file_image_tmp,$file_image_old)
	{
		...
		$h = 1080;					
 		...
 		$this->image_moo->load($file_image_tmp)->resize_crop($w,$h)->save("../uploads/abouts/".$file_image);
		$this->image_moo->load($file_image_tmp)->resize_crop(200,100)->save("../uploads/abouts/thumbs/".$file_image);
	...
	{
		if(!empty($file_image)){
			if(file_exists("../uploads/online-services/".$file_image)){ unlink("../uploads/online-services/".$file_image); }
			if(file_exists("../uploads/online-services/medium/".$file_image)){ unlink("../uploads/online-services/medium/".$file_image); }
			if(file_exists("../uploads/online-services/thumbs/".$file_image)){ unlink("../uploads/online-services/thumbs/".$file_image); }
		}
	}
