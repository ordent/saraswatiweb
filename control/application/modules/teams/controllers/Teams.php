...
class Teams extends MX_Controller  {
	...
	{	
		...
		$jum_record = $this->teams->getTotal($sch_pack);
		...
		#record
		$query = $this->teams->getList($per_page,$lmt,$sch_pack);
		$list = array();
	...
	function edit_content($id)
	{
		...
		if(is_numeric($id)){
			...
			if($q->num_rows() > 0){
					...
					{
						$file_image = "
						<a href='".asset_url()."teams/".$r->file_image."' rel='facebox'><img src='".asset_url()."teams/thumbs/".$r->file_image."' class='thumbnail'></a>
						<br/>
					...
					$job = $this->session->flashdata("job") ? $this->session->flashdata("job") : $r->job;
					...
					$job = $this->session->flashdata("job") ? $this->session->flashdata("job") : "";
	...
	function submit()
	{
		...
		if(!empty($file_image_title)){
			if(!empty($file_image_old)){
				$this->teams->deleteFileUpload($file_image_old);
			}
			$file_image = $this->teams->setFileUpload($file_image_title,$file_image_tmp,$file_image_old);
		}else{
		...
		if ($this->form_validation->run($this) == FALSE)
		{
			...
			if($id > 0)
			{
				$this->teams->setUpdate($data_pack,$where_pack);
				$this->session->set_flashdata("success","Data saved successful");
				$link_id = $id;
			}else{
				$id_term = $this->teams->setInsert($data_pack);
	...
	{
		$product = $this->teams->getDetail($id);
		$row = $product->row();
		$file_image = $row->file_image;
		$del_images = $this->teams->setPictureDelete($id,$file_image);
		$del_status = $this->teams->setDelete($id);
		$response['id'] = $id;
	...
	function unlink($id,$file_image)
	{
		...
		$this->teams->setPictureDelete($id, $file_image);
		redirect($this->path."/edit/".$id);
	}
