...
class Ref_socials extends MX_Controller  {
	
	var $path = "ref_socials";
	var $alias = "Social Links";	
	var $uri_page = 6;
	var $per_page = 25;
	...
	{	
		...
		$jum_record = $this->ref_socials->getTotal($sch_pack);
		...
		#record
		$query = $this->ref_socials->getList($per_page,$lmt,$sch_pack);
		$list = array();
		...
		if($query->num_rows() > 0){
			foreach($query->result() as $r)
			{
				$no++;
				$name = $r->name;
				$name = highlight_phrase($name, $sch1_parm, '<span style="color:#990000">', '</span>');
	...
	function edit_content($id)
	{
		...
		if(is_numeric($id)){
			...
			#record
			$q = $this->ref_socials->getDetail($id);
			...
			if($q->num_rows() > 0){
					...
					$class = $this->session->flashdata("class") ? $this->session->flashdata("class") : $r->class;
					...
					$class = $this->session->flashdata("class") ? $this->session->flashdata("class") : "";
					$id = 0;
	...
	function submit()
	{
		...
		if ($this->form_validation->run($this) == FALSE)
		{
			...
			if($id > 0)
			{
				$this->ref_socials->setUpdate($data_pack,$where_pack);
				$this->session->set_flashdata("success","Data saved successful");
				redirect($this->path."/edit/".$id);
			}else{
				$id_term = $this->ref_socials->setInsert($data_pack);
	...
	{
		$del_status = $this->ref_socials->setDelete($id);
		$response['id'] = $id;
