...
class Model_contacts extends CI_Model {
	...
	function setFileUpload($file_image,$file_image_tmp,$file_image_old)
	{
		...
		$h = 435;					
 		...
 		$this->image_moo->load($file_image_tmp)->resize_crop($w,$h)->save("../uploads/contacts/".$file_image);
		$this->image_moo->load($file_image_tmp)->resize_crop(145,235)->save("../uploads/contacts/thumbs/".$file_image);
	...
	{
		if(!empty($file_image)){
			if(file_exists("../uploads/contacts/".$file_image)){ unlink("../uploads/contacts/".$file_image); }
			if(file_exists("../uploads/contacts/thumbs/".$file_image)){ unlink("../uploads/contacts/thumbs/".$file_image); }
		}
	}
