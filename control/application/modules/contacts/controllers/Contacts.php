...
class Contacts extends MX_Controller  {
	...
	{	
		...
		$jum_record = $this->contacts->getTotal($sch_pack);
		...
		#record
		$query = $this->contacts->getList($per_page,$lmt,$sch_pack);
		$list = array();
	...
	function edit_content($id)
	{
		...
		if(is_numeric($id)){
			...
			#record
			$q = $this->contacts->getDetail($id);
			$list = $list_term_option = array();
			if($q->num_rows() > 0){
					$r = $q->row();
					
					$title = $this->session->flashdata("name") ? $this->session->flashdata("name") : $r->name;
					$phone = $this->session->flashdata("phone") ? $this->session->flashdata("phone") : $r->phone;
					$email = $this->session->flashdata("email") ? $this->session->flashdata("email") : $r->email;
					$address = $this->session->flashdata("address") ? $this->session->flashdata("address") : $r->address;
					...
					$phone = $this->session->flashdata("phone") ? $this->session->flashdata("phone") : "";
					$email = $this->session->flashdata("email") ? $this->session->flashdata("email") : "";
					$address = $this->session->flashdata("address") ? $this->session->flashdata("address") : "";
					$id = 0;
	...
	function submit()
	{
		...
		if ($this->form_validation->run($this) == FALSE)
		{
			...
			if($id > 0)
			{
				$this->contacts->setUpdate($data_pack,$where_pack);
				$this->session->set_flashdata("success","Data saved successful");
				$link_id = $id;
			}else{
				$id_term = $this->contacts->setInsert($data_pack);
	...
	{
		$product = $this->contacts->getDetail($id);
		$row = $product->row();
		$del_status = $this->contacts->setDelete($id);
		$response['id'] = $id;
